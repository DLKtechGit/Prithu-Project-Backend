...
exports.getAllFeeds = async (req, res) => {
  try {
    ...
    const interactionsAgg = await UserFeedActions.aggregate([
      ...
      { $project: { feedId: 1, likedFeeds: 1, sharedFeeds: 1, downloadedFeeds: 1 } }
    ]);
    ...
    // 6. Map accounts, creators, admins
    ...
    const enrichedFeeds = feeds.map(feed => {
      ...
      if (feed.roleRef === "Account" && feed.createdByAccount) {
        ...
        profileAvatar = acc?.userId?.profileSettings?.profileAvatar
          ? `${host}/uploads/images/${path.basename(acc.userId.profileSettings.profileAvatar)}`
          : "Unknown";
        ...
        profileAvatar = cre?.userId?.profileSettings?.profileAvatar
          ? `${host}/uploads/images/${path.basename(cre.userId.profileSettings.profileAvatar)}`
          : "Unknown";
        ...
        profileAvatar = adm?.profileSettings?.profileAvatar
          ? `${host}/uploads/images/${path.basename(adm.profileSettings.profileAvatar)}`
          : "Unknown";
      }
      ...
      return {
        ...
        commentsCount: commentCountMap[feed._id.toString()] || 0,
